 ...
 *   <img class="lp_browser_img"> + <canvas class="lp_browser_water">.
 */
(() => {
  'use strict';
  ...
  const SIM_RES = 256;
  const DAMPING = 0.99;
  const DISPLACEMENT = 0.04;
  const DROP_RADIUS = 0.03;
  const DROP_ASPECT_Y = 1.8;
  const DROP_STRENGTH = 0.16;
  const GLOW_DAMPING = 0.07;
  const DROP_GAP_PX = 30;
  const STEP_EVERY = 2; // advance the wave sim every Nth frame -> slower, calmer ripples
...
void main() {
  ...
  class WaterRipple {
    ...
    tick = () => {
      ...
      // 2. One simulation step, advanced every Nth frame for a slower, calmer ripple.
      this.stepCounter = (this.stepCounter || 0) + 1;
      if (this.stepCounter % STEP_EVERY === 0) {
  ...
  function attach(card) {
    const img = card.querySelector('.lp_browser_img');
    const canvas = card.querySelector('.lp_browser_water');
